...
describe('seeding is idempotent and reconciling', () => {
  ...
  it('withdraws a grant that no longer exists in code', async () => {
    ...
    const { rows } = await c.query(`SELECT id FROM role WHERE key = 'coordinator'`);
    await c.query(
...
describe('structural guarantees survive the round trip to the database', () => {
  it('no role in the database can approve a graduation except System Admin', async () => {
    const { rows } = await c.query(
      `SELECT r.key FROM role_permission rp JOIN role r ON r.id = rp.role_id
       WHERE rp.module = 'graduation' AND rp.verb = 'approve'`,
    );
    expect(rows.map((r) => r.key).sort()).toEqual(['project_manager', 'system_admin']);
  });
